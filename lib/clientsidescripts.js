...
clientSideScripts.findRepeaterColumn = function() {
  ...
  for (var i = 0; i < rows.length; ++i) {
    ...
    // find the ng-model bindings
    var allElements = rows[i].getElementsByTagName('*');
    for (var l = 0; l < allElements.length; l++) {
      if (allElements[l].getAttribute('ng-model')) {
        if (allElements[l].getAttribute('ng-model') === binding)
          matches.push(allElements[l]);
      }
    }
  }
  return matches;
